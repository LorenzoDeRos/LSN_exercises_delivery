...
{
	return 1.0;
	//return exp(-pow((v-mu)/sigma, 2.)/2.) + exp(-pow((v+mu)/sigma, 2.)/2.);
}

double variationalWaveFunction_second(double v)
{
	return 0;
	//return -variationalWaveFunction(v)/sigma/sigma + pow((v-mu)/sigma/sigma, 2.)*exp(-pow((v-mu)/sigma, 2.)/2.) + pow((v+mu)/sigma/sigma, 2.)*exp(-pow((v+mu)/sigma, 2.)/2.);
}
