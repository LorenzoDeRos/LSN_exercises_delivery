...
int main()
{ 
  ofstream outepot, outpres;
  outepot.open("epot_allvalues.dat");
  outpres.open("pres_allvalues.dat");
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
      outepot << walker[iv]/(double)m_part +vtail << endl;
      outpres << rho*temp+(walker[iw]+ptail*m_part)/vol << endl;
...
//g(r)
    for(int k=igofr; k<igofr+nbins; ++k) {
      delta_vol=4.*M_PI/3.*(pow((k-1.)*bin_size, 3.)-pow((k-2.)*bin_size, 3.));
      gdir=blk_av[k]/rho/m_part/delta_vol/blk_norm;
      glob_av[k] += gdir;
      glob_av2[k] += gdir*gdir;
      err_gdir=Error(glob_av[k], glob_av2[k], iblk);
      Gofr << setw(wd) << iblk << setw(wd) << (k-igofr)*bin_size << setw(wd) << gdir << setw(wd) << glob_av[k]/(double)iblk << setw(wd) << err_gdir << endl;
      if(iblk==nblk) {
        Gave << (k-igofr)*bin_size << " " << glob_av[k]/(double)iblk << " " << err_gdir << endl;
      }
    }
